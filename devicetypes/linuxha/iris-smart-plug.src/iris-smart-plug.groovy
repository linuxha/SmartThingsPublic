...
metadata {
	...
	definition (name: "Iris Smart Plug", namespace: "linuxha", author: "SmartThings") {
		capability "Actuator"
		capability "Switch"
...
def calculateAndShowEnergy() {
    ...
    // Hmm, why is this null ?
    
    if(energyValue == null) {
    	energyValue = 0
    }

	log.debug "recentEvents = " + recentEvents[1].value
    log.debug "energyValue  = " + energyValue
    energyValue += (recentEvents[1].value * deltaT) / 1000 // energy used since last "power" event in kWh

	log.debug "recentEvents = " + recentEvents.flatten()
